...
- `static int read(uint32_t offset, uint8_t *buf, size_t size)`：读取操作
...
- `static int write(uint32_t offset, const uint8_t *buf, size_t size)` ：写入操作
...
- `static int erase(uint32_t offset, size_t size)` ：擦除操作
